...
describe('Idempotency E2E tests, wrapper function usage', () => {
  ...
  it('creates a DynamoDB item with the correct attributes', async () => {
    // Prepare
    const payload = {
      records: [
        { foo: 'bar', id: 1 },
    ...
    const idempotencyRecordsItems = [
      idempotencyRecords.Items?.find(
        (record) =>
          record.customId === `${functionNameCustomConfig}#${payloadHashes[0]}`
      ),
      idempotencyRecords.Items?.find(
        (record) =>
          record.customId === `${functionNameCustomConfig}#${payloadHashes[1]}`
      ),
      idempotencyRecords.Items?.find(
        (record) =>
          record.customId === `${functionNameCustomConfig}#${payloadHashes[2]}`
      ),
    ];
  ...
  it('calls the wrapped function once and always returns the same result when called multiple times', async () => {
    // Prepare
    const payload = {
      body: JSON.stringify({
        foo: 'bar',
