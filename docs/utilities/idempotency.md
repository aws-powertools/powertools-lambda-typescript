...
| **payloadValidationJmespath** | `''`        | JMESPath expression to validate whether specific parameters have remained the same for a matched idempotency key across multiple requests (e.g., to ensure consistency in critical fields like transaction amounts, or to detect tampered payloads) |
