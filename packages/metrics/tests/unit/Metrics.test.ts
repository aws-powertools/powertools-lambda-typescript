...
describe('Metrics', () => {
  ...
  test('Metadata should be added correctly', () => {
    ...
    const newLoggedData = metrics.serializeMetrics();
    expect(newLoggedData[metadataItem.name]).toBeUndefined();
  ...
  test('Clearning metrics should return empty', () => {
    const metrics = new Metrics({ namespace: 'test' });
    ...
    class LambdaFunction implements LambdaInterface {
      ...
      public handler<TEvent, TResult>(_event: TEvent, _context: Context, _callback: Callback<TResult>): void | Promise<TResult> {
        ...
        const newLoggedData = metrics.serializeMetrics();
        expect(newLoggedData._aws.CloudWatchMetrics[0].Metrics.length).toEqual(0);
      }
    }
    new LambdaFunction().handler(dummyEvent, dummyContext, () => console.log('Lambda invoked!'));
